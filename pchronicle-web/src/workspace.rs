...
fn run_from_search(search: &str) -> Option<RunSummary> {
    let agent_id = query_value(search, "agent_id").filter(|value| !value.is_empty())?;
    let session_id = query_value(search, "session_id").filter(|value| !value.is_empty())?;
    Some(RunSummary {
        dataset: query_value(search, "dataset").unwrap_or_default(),
        file: query_value(search, "file").unwrap_or_default(),
        run_id: query_value(search, "run_id").filter(|value| !value.is_empty()),
        agent_id,
        model_name: None,
        session_id,
...
fn workspace_route_page(search: &str) -> &'static str {
    page_from_query(
        query_value(search, "page").as_deref(),
        run_from_search(search).is_some(),
    )
}

fn catalog_nav_from_search(search: &str) -> Option<(String, String)> {
    if workspace_route_page(search) != "catalog" {
        return None;
    }
    Some((
        query_value(search, "dataset").unwrap_or_default(),
        query_value(search, "prefix").unwrap_or_default(),
    ))
}

fn workspace_place_key(
    page: &str,
    catalog_dataset: &str,
    catalog_prefix: &str,
    dataset_filter: &str,
    ...
    match page {
        "home" => "home".into(),
        "catalog" => format!("catalog|{catalog_dataset}|{catalog_prefix}"),
        "runs" => format!("runs|{dataset_filter}|{file_prefix}|{run_path}"),
        "detail" => {
            let (dataset, file, session) = run
                .map(|run| {
                    (
                        run.dataset.as_str(),
                        run.file.as_str(),
                ...
                .unwrap_or_default();
            format!("detail|{dataset}|{file}|{session}")
        }
        "tools" => "tools".into(),
...
fn should_push_workspace_history(
    history_ready: bool,
    previous_place: &str,
    next_place: &str,
    same_url: bool,
) -> bool {
    history_ready && previous_place != next_place && !same_url
}

fn navigate_app_back(app_history_depth: i32) -> bool {
    if app_history_depth <= 0 {
        return false;
    }
    web_sys::window()
...
pub fn App() -> Element {
    ...
    let url_dataset = url_param("dataset").unwrap_or_default();
    let initial_file = url_param("file").unwrap_or_default();
    let url_prefix = url_param("prefix").unwrap_or_default();
    ...
    let mut catalog_dataset = use_signal({
        let value = if initial_page == "catalog" {
            url_dataset
        } else {
            String::new()
        };
    ...
    let mut catalog_prefix = use_signal({
        let value = if initial_page == "catalog" {
            url_prefix
        } else {
            String::new()
    ...
    use_effect(move || {
        let next_place = workspace_place_key(
            &page(),
            &catalog_dataset(),
            &catalog_prefix(),
            &dataset_filter(),
            &file_prefix(),
            &run_path(),
            selected_run().as_ref().filter(|_| page() == "detail"),
        );
        let href = workspace_href(
            &page(),
            &analysis_session_id(),
        ...
        sync_workspace_history(next_place, href, last_place, history_ready, history_seq);
    });
    ...
    use_hook(|| {
        ...
        let callback = Closure::<dyn FnMut()>::new(move || {
            ...
            history_seq.set(history_seq_from_window());
                        ...
                        rsx! { div { class: "pc2-detail-layout",
                            ...
                            if let (Some(_run), Some(value)) = (selected_run(), analysis()) {
                                RunDetailWorkspace {
                                    ...
                                    query: turn_query(),
                                    on_back: move |_| {
                                        if !navigate_app_back(history_seq()) {
                                            page.set("runs".into());
                                        }
                                    },
...
fn normalize_search(url: &str) -> &str {
    url.split_once('?')
        .map(|(_, search)| search)
        .unwrap_or_else(|| url.trim_start_matches('/'))
}

fn workspace_url_matches_location(url: &str) -> bool {
    let Some(window) = web_sys::window() else {
        return false;
    };
    let Ok(search) = window.location().search() else {
    ...
    normalize_search(&search) == normalize_search(url)
}

fn write_workspace_history(url: &str, push: bool, seq: i32) {
    let Some(window) = web_sys::window() else {
        return;
    };
    ...
    let state = JsValue::from(f64::from(seq));
    if push {
...
fn sync_workspace_history(
    next_place: String,
    href: Option<String>,
    mut last_place: Signal<String>,
    mut history_ready: Signal<bool>,
    mut history_seq: Signal<i32>,
) {
    let same_url = href
        .as_ref()
        .is_some_and(|url| workspace_url_matches_location(url));
    // These are bookkeeping outputs, not dependencies of the route effect.
    // Subscribing to them would make each write schedule the effect again.
    let push = should_push_workspace_history(
        *history_ready.peek(),
        &last_place.peek(),
        &next_place,
        same_url,
    ...
    if let Some(url) = href
        && !same_url
    {
        let seq = *history_seq.peek() + i32::from(push);
        write_workspace_history(&url, push, seq);
        if *history_seq.peek() != seq {
            history_seq.set(seq);
        }
    }
    // Signal::set notifies every subscriber even when the value is unchanged.
    // Writing only on change keeps a future tracked read from spinning the
    // effect (and blocking the first paint) after bookkeeping has settled.
    if last_place.peek().as_str() != next_place {
        last_place.set(next_place);
    }
    if !*history_ready.peek() {
        history_ready.set(true);
    }
}

fn history_seq_from_window() -> i32 {
    web_sys::window()
        .and_then(|window| window.history().ok())
...
#[allow(clippy::too_many_arguments)]
fn apply_workspace_search(
    search: &str,
    mut page: Signal<String>,
    mut catalog_dataset: Signal<String>,
    mut catalog_prefix: Signal<String>,
    mut dataset_filter: Signal<String>,
    ...
    mut selected_run: Signal<Option<RunSummary>>,
    mut analysis: Signal<Option<RunAnalysis>>,
    mut analysis_session_id: Signal<String>,
    mut detail_mode: Signal<String>,
    mut trace_mode: Signal<String>,
    mut source: Signal<String>,
    mut turn_query: Signal<String>,
    mut expanded_turn_id: Signal<Option<i64>>,
) {
    let next_page = workspace_route_page(search);
    if next_page == "catalog"
        && let Some((dataset, prefix)) = catalog_nav_from_search(search)
    {
        catalog_dataset.set(dataset);
        catalog_prefix.set(prefix);
    }
    if next_page == "runs" {
        dataset_filter.set(query_value(search, "dataset_filter").unwrap_or_else(|| "all".into()));
    ...
    if next_page == "detail" {
        if let Some(run) = run_from_search(search) {
            selected_run.set(Some(run));
            analysis.set(None);
        }
        detail_mode.set(query_value(search, "workspace").unwrap_or_else(|| "trace".into()));
        trace_mode.set(
            normalize_trace_view(&query_value(search, "view").unwrap_or_else(|| "chats".into()))
                .to_string(),
        );
        source.set(query_value(search, "source").unwrap_or_else(|| "all".into()));
        turn_query.set(query_value(search, "turn_q").unwrap_or_default());
        expanded_turn_id
            .set(query_value(search, "turn").and_then(|value| value.parse::<i64>().ok()));
    }
    if next_page == "tools" {
        analysis_session_id.set(query_value(search, "analysis_session").unwrap_or_default());
    }
    page.set(next_page.to_string());
}
...
mod tests {
    ...
    fn catalog_href(dataset: &str, prefix: &str) -> String {
        workspace_href(
            "catalog", "", false, None, "", "all", "all", "session", "asc", "", "", dataset,
            prefix, "trace", "chats", "all", "", None,
        )
        .expect("catalog href")
    }

    #[test]
    fn catalog_url_keeps_dataset_and_prefix() {
        assert_eq!(catalog_href("", ""), "/?page=catalog");
        assert_eq!(catalog_href("live", ""), "/?page=catalog&dataset=live");
        assert_eq!(
    ...
    fn drain_effects(dom: &mut VirtualDom) {
    ...
    #[test]
    fn history_sync_effect_settles_after_recording_a_route() {
        ...
        let mut dom = VirtualDom::new_with_props(
            |calls: Rc<Cell<usize>>| {
                let last_place = use_signal(String::new);
                let history_ready = use_signal(|| false);
                let history_seq = use_signal(|| 0);
                use_effect(move || {
                    calls.set(calls.get() + 1);
                    sync_workspace_history(
                        "home".into(),
                        None,
                        last_place,
                        history_ready,
        ...
        drain_effects(&mut dom);
        assert_eq!(
            calls.get(),
            1,
            "history bookkeeping must not retrigger the effect"
        );
    }

    #[test]
    fn catalog_search_restores_browse_depth() {
        assert_eq!(
            catalog_nav_from_search("?page=catalog&dataset=live&prefix=nested%2Fchild"),
            Some(("live".into(), "nested/child".into()))
        );
        assert_eq!(
            catalog_nav_from_search("?page=runs&dataset_filter=live&file_prefix=nested"),
            None
        );
        assert_eq!(
            workspace_route_page("?page=catalog&dataset=live"),
            "catalog"
        );
    }

    #[test]
    fn workspace_place_treats_catalog_depth_as_history() {
        assert_ne!(
            workspace_place_key("catalog", "live", "nested", "all", "", "", None),
            workspace_place_key("catalog", "live", "", "all", "", "", None)
        );
        assert_ne!(
            workspace_place_key("catalog", "live", "nested", "all", "", "", None),
            workspace_place_key("runs", "live", "nested", "live", "nested", "", None)
        );
        assert!(should_push_workspace_history(
            true,
            "catalog|live|nested",
            "runs|live|nested|",
            false
