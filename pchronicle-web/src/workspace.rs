...
pub fn App() -> Element {
    ...
    let initial_run_id = url_param("run_id").unwrap_or_default();
...
fn sync_workspace_url(
    ...
    if let Some(run) = run.filter(|_| page == "detail") {
        ...
        params.push(format!("run_id={}", urlencoding::encode(&run.run_id)));
...
mod tests {
    ...
    fn run_at(path: &str) -> RunSummary {
        RunSummary {
            ...
            file: ".".into(),
            run_id: path.rsplit('/').next().unwrap_or_default().into(),
            agent_id: "agent".into(),
            model_name: None,
