...
cleanup() {
  set +e
  if [[ -n "${STORAGE:-}" ]]; then
    "$CLI" capture stop -o "$STORAGE" >/dev/null 2>&1 || true
  fi
  if [[ -n "${MOCK_PID:-}" ]]; then
...
if ! run_cli capture start --help >/dev/null 2>&1; then
  die "binary missing 'capture start' — rebuild: cargo build -p persisting-cli"
fi
expect_grep "capture help lists subcommands" "start|stop|list|status" capture --help
expect_grep "capture import help" "provider|gateway|ide" capture import --help

expect_fail "capture start without config" capture start -o "$STORAGE"
expect_fail "capture serve without output dir" capture serve -c "$CONFIG"

# --- 2. import (no daemon) ------------------------------------------------
section "capture import (dry-run, gateway JSONL)"

expect_grep "import dry-run finds gateway records" "capture-it-agent|gw-session|imported|dry-run" \
  capture import "$STORAGE" \
  --provider gateway \
...
expect_fail "gateway provider without readable input" \
  capture import "$STORAGE" --provider gateway --gateway-input /nonexistent/capture-it.jsonl

# --- 3. mock upstream + daemon start ----------------------------------------
section "mock upstream and capture start"
...
run_cli capture start -o "$STORAGE" -c "$CONFIG" >/dev/null || die "capture start failed"
pass "capture start"
...
# --- 4. status / list -------------------------------------------------------
section "capture status and list"
...
run_cli capture status -o "$STORAGE" >/dev/null || die "capture status failed"
pass "capture status CLI"

LIST_OUT="$(run_cli capture list -o "$STORAGE")"
if grep -q 'No capture sessions found' <<<"$LIST_OUT"; then
  pass "capture list empty before proxy traffic"
else
  grep -q 'agent_id' <<<"$LIST_OUT" || { echo "$LIST_OUT"; die "capture list missing table header"; }
  pass "capture list table header"
fi
...
START2_OUT="$(run_cli capture start -o "$STORAGE" -c "$CONFIG" || true)"
...
sleep 0.5
LIST2="$(run_cli capture list -o "$STORAGE")"
grep -q "$SESSION_ID" <<<"$LIST2" || { echo "$LIST2"; die "capture list missing session after request"; }
pass "capture list shows session after proxy call"

# --- 7. capture run (in-process proxy, no forked daemon) --------------------
section "capture run (subprocess + env)"

RUN_STORAGE="$WORKDIR/run-store"
mkdir -p "$RUN_STORAGE"
RUN_OUT="$(run_cli capture run -o "$RUN_STORAGE" -c "$CONFIG" --session-id run-it-session -- true)"
grep -q 'capture run' <<<"$RUN_OUT" || true
pass "capture run executed true"

[[ -f "$RUN_STORAGE/.capture/run_session" ]] || die "missing run_session file"
grep -q 'run-it-session' "$RUN_STORAGE/.capture/run_session" || die "run_session content mismatch"
pass "run_session file"

# --- 8. storage resolution via env ------------------------------------------
section "PERSISTING_CAPTURE_STORAGE resolution"

export PERSISTING_CAPTURE_STORAGE="$STORAGE"
run_cli capture list >/dev/null || die "capture list with PERSISTING_CAPTURE_STORAGE failed"
unset PERSISTING_CAPTURE_STORAGE
pass "list without positional STORAGE (env)"

# --- 9. stop ----------------------------------------------------------------
section "capture stop"

run_cli capture stop -o "$STORAGE" >/dev/null || die "capture stop failed"
pass "capture stop"

sleep 0.3
[[ ! -f "$(daemon_json)" ]] && pass "daemon.json removed" || die "daemon.json still present after stop"

expect_fail "status after stop" capture status -o "$STORAGE"

if run_cli capture start -o "$STORAGE" -c "$CONFIG" >/dev/null; then
  pass "restart after stop"
  run_cli capture stop -o "$STORAGE" >/dev/null || die "stop after restart"
fi
