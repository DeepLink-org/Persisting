...
pPilot 用一个约束给你这一切：写一个 Python 文件，两个函数——`plan()`（做什么）和 `execute(item)`（怎么做）。

---

## 你的第一个任务

一个 pPilot 任务就是一个 Python 文件。`plan()` yield 任务 dict。`execute(item)` 处理一条并返回结果。
...
和 `retryable`；pPilot 会额外写入包含 count/sum/mean 指标聚合的 `summary.json`。
...
- [pPilot 架构](../design/ppilot.md) — Driver、调度器、sink、quarantine
- [示例脚本](https://github.com/DeepLink-org/Persisting/tree/main/examples/ppilot)
