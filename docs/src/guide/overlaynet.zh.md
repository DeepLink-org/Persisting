...
彻底阻止直接联网，应改用 `--container-network none` 这样的 executor 级边界，而不是
OverlayNet 策略。当前 KVM executor 还不能使用宿主机的 OverlayNet endpoint；container
executor 使用进程内代理时要求 `--container-network host`。
