...
从 Lance 数据集开始——你的文档。从 JSONL、CSV 或已有的 Lance 表导入：

```bash
# 从 JSONL（每行一个 JSON 对象，必须有 "text" 字段）
...
```bash
persisting search query ./docs "如何配置代理" --mode hybrid --k 10
```
...
向量化步骤（`add_document`）调用 `persisting-engine`（Rust），生成向量并与文本一起写入 Lance 表。索引构建和查询也在 Rust 中通过引擎运行，由 CLI 运行时动态加载。
...
- [Search CLI 设计](../design/cli-search.md) — 命令结构与设计选择
