...
pPilot gives you all of that with one constraint: write a Python file with two functions — `plan()` (what to do) and `execute(item)` (how to do it).

---

## Your First Task

A pPilot task is a single Python file. `plan()` yields task dicts. `execute(item)` processes one and returns a result.
...
`error_kind`, and `retryable`; pPilot also writes `summary.json` with metric
...
```bash
persisting ppilot plan.py --sink runs/exp-01 --resume --rerun-failed infra
```
...
# Does it even work?
python3 task.py --n 2
persisting ppilot task.py --check -- --n 2

# Run with 4 local workers
persisting ppilot task.py -w 4 -- --n 100

# Or fewer workers, more concurrency per worker
persisting ppilot task.py -w 1 --per-worker 4 -- --n 100
```

The same file scales to multi-process without changes:

```bash
torchrun --nproc_per_node=8 -- persisting ppilot task.py -- --n 100
```
...
```bash
persisting ppilot task.py -w 4 --sink /tmp/run1 -- --n 1000
```
...
```bash
persisting ppilot task.py -w 4 --sink /tmp/run1 --resume -- --n 1000
```
...
```bash
persisting ppilot task.py -w 4 --sink /tmp/run1 --resume --observe -- --n 1000
```
...
The pPilot runtime follows a simple model:
...
Use pPilot when you want `plan()` + `execute()` without bringing in a distributed framework. Best for file/shard-level tasks. Not for per-row dispatch.

---

## Next Steps

- [pPilot Architecture](../design/ppilot.md) — Driver, scheduler, sink, quarantine
- [Example scripts](https://github.com/DeepLink-org/Persisting/tree/main/examples/ppilot)
