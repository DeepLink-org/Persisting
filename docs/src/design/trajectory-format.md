...
> **Capture 写入互斥**：`-f md` 时 Proxy 内 CaptureEngine 按 `call_id`+`role` **upsert** 块到对应 md（主 session → `run-{run_id}.md`，subagent → `agent-{id}.md`），流式 assistant 块原地 rewrite，并刷新 **YAML frontmatter**；run 结束写 `reconcile.json`。`-f lance` **只** append `{run}/events.lance/`（Lance dataset 目录），**不** live 写 md；人读视图用 `traj materialize`。
...
Markdown 路径解析见 [轨迹存储模型 §7.3](trajectory.md)。
...
| **有损物化** | 相对 Lance 省略内部流量与 lifecycle；见 [存储模型 §1](trajectory.md) |
...
Import 路径仍使用批量 **append**。详见 [轨迹存储 §5.3](trajectory.md)。
...
| `seq` | 推荐 | Lance 行序号（物化块通常与 canonical 一致） |
...
当请求或响应含图像时，**Markdown 块正文与 Lance 摘要字段**使用占位符，不内联 base64 像素（避免 git diff 与 frontmatter 膨胀）。
...
| Capture `-f lance` | 仅 Lance（`events.lance/` 目录） |
| `traj materialize` | 全量扫描 Lance，**重写**指定 session 的 Markdown（修复/补全；traj capture 需 `--root-session-id`） |
| `traj add` + markdown 存储 | **只写一层**（`--storage-format markdown`）；需要 Lance 时再单独 `traj compact` / append |
| `traj add` + 输入为 `.md` | 解析 TLV → compact/append 到 Lance |
| `traj replay` | 有 Lance 时读 Lance；纯 md session 从块还原 |
...
- [Traj 命令](cli-traj.md) · [Capture 子命令](cli-capture.md)
- [`persisting trajectory` 命令](cli-traj.md)
