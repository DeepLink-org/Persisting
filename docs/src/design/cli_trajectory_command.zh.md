...
| **stats** | 双层摘要；`--detail` 逐轮树；未指定 `--session-id` 时扫描 agent 下全部 run，并对 run bucket **展开** Vortex 内 distinct `session_id` 后逐分区统计（见 [轨迹存储 §7.1.1](trajectory_storage.zh.md#711-run-bucket-内多-session_id-分区)） |
| **replay** | 分页输出事件 JSON |
