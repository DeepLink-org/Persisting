...
| `events` | 每个 canonical event 一行 | 仅 `events.lance` source |
| `trajectories` | 每个 `runs` 行一行，聚合步骤与工具摘要 | Catalog view |

`runs`、`steps`、`tool_calls` 的业务列与
[Storyline 三表 schema](storyline-lance.md)一致，Catalog 只在首列增加 `_file_`。
`events` 同样在 canonical event schema 前增加 `_file_`。外围文件不会生成伪造的原始
event 行；它们只能通过 Storyline 规范化表查询。`trajectories` 按
`(_file_, session_id)` 聚合，包含 step/tool 数量以及有序摘要数组。
...
| `format` | UTF-8, nullable | `storyline`、`events`、`atif`、`openai_msg` 或 `actf` |
