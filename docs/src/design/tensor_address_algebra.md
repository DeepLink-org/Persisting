# Tensor Address Algebra (TAA) —— Persisting 分布式分层内存的寻址模型
...
TAA 是 Persisting 数据面的**寻址模型**：定义"数据在多维地址空间中如何定位"，驱动分层 placement、跨节点路由和批量优化。

### 0.2 与 PGAS 的对比：TAA 提供了 PGAS 做不到的

PGAS（UPC、Chapel）为 HPC 提供了全局地址空间，但受限于：单一扩展维（rank）、线性地址、shmem API、无分层。AI workload 需要不同的东西：
...
PGAS 在 HPC 失败的教训：仅靠"更好的地址模型"不能推动帕累托曲线。Persisting 从中吸取的经验：**TAA 是内部寻址基础设施，对外不卖"统一地址代数"——价值用 KV Cache P99、GPU 利用率、内存效率来衡量。**

### 0.3 TAA 解决什么

TAA 为 Persisting 的三类内存场景提供统一寻址：
...
- 物理连续字节偏移假设
- 分层 placement policy 的在线决策（由 Persisting 的 tiering 层负责，TAA 只提供地址信息）

用户看到的是 `kv["s1", 0, 2, 0:512].tensor()`。TAA 在底层驱动寻址和路由，用户不需要直接操作。
...
本节用于把本设计在项目背景下讲清楚：**不是“通用 DB”**，而是一套面向 AI workload 的受限查询形态 + 确定性规范化与 lowering 规范。

### 11.1 设计主张（范围、收益、可验证性）

- **统一表示**：受限的多维地址/约束表示（`Address/Region` + `Point/Range/SetC`）覆盖训练/推理/RL 的关键访问形态：点查、批量点查、单维范围扫描、滑窗预取。
- **确定性优化**：对该表示做确定性规范化与批量合并，可在不引入代价模型/计划搜索的前提下，显著降低扇出（fan-out）、RPC 次数与 IO ops，并改善尾延迟。
- **可验证性**：在明确的 key-order 假设下，规范化与 lowering 的正确性可以形式化陈述，并用 property-based 测试覆盖，无需构建通用查询引擎。

### 11.2 应用覆盖表：三类存储场景如何落到同一套 NF 与原语

下表强调“统一点”是 **访问形态与编译链路**，不是“我们支持任意查询”。

| 场景 | 典型逻辑维度（示例） | 典型访问形态 | 归约到的 NF | Lowering 后端原语 | 主要可量化收益指标 |
|------|----------------------|--------------|------------|-------------------|-------------------|
| **KV Cache（推理）** | `(session, layer, head, time)` | 大量点查/批点查；按 `time` 单维 range；滑窗预取 | PointQuery / RangeQuery | `mget` / `range_scan` / `prefetch` | IO ops、RPC 次数、P99/P999、CPU 开销 |
| **PS / Optimizer state（训练）** | `(param_id, shard, step?)` | 批量点查/批写（参数块、梯度块） | PointQuery（为主） | `mget` / `put_batch` | RPC 次数、吞吐（items/s）、CPU 开销 |
| **轨迹存储（RL）** | `(run_id, time)` | 按 `run_id` 前缀 + `time` 单维 range_scan；批量读取多个 run | RangeQuery（为主） | `range_scan`（批量合并后更少） | 扫描吞吐、IO ops、P99 |

说明：

- “弹性（elastic）训练/推理/RL”在这里的含义不是营销词，而是可被实验验证的两类现象：
  - **扇出（fan-out）控制**：规范化与按 `partition_dims` 分组后，单个 batch 触达的 shard 数显著下降
  - **混部尾延迟**：在推理（读多、尾延迟敏感）与训练/RL（写多/scan 多）共存时，P99/P999 更稳定

### 11.3 正确性陈述（只对受限子语言负责）

本设计只对第 4 章定义的 NF 负责。下面给出可作为实现验收与测试依据的正确性陈述。

#### 定义：满足关系与语义

令 `Sat(a, d, c)` 表示地址 `a` 在维度 `d` 上满足约束 `c`。则：

\[
\llbracket R \rrbracket = \{ a \mid \forall (d \mapsto c) \in R.constraints,\; Sat(a,d,c) \}
\]

#### 命题 1：`meet` 的正确性（约束交）

对任意单维约束 `c1,c2`，若 `meet(c1,c2) = c`，则：

\[
\llbracket c \rrbracket = \llbracket c1 \rrbracket \cap \llbracket c2 \rrbracket
\]

若 `meet(c1,c2)=Empty`，则交集为空。

#### 命题 2：规范化幂等与语义保持

令 `canon(R)` 为第 4.2 节的规范化过程输出（或 Empty）。

- **幂等**：`canon(canon(R)) = canon(R)`
- **语义保持**：若 `canon(R)` 非 Empty，则
  \[
  \llbracket canon(R) \rrbracket = \llbracket R \rrbracket
  \]

#### 命题 3：对 NF 的 lowering 完备性（在 key-order 假设下）

假设集合 schema 满足第 1 章的 `KeyOrder/order_dim` 约束，且 `encode_key` 保持字典序可扫描性。则对任意 NF 查询 `R`：

- 若 `R` 是 **NF-PointQuery**，lowering 到 `mget([encode_key(addr)])` 完备且正确；
- 若 `R` 是 **NF-RangeQuery**，lowering 到 `range_scan(prefix, lo, hi)` 完备且正确（返回的 key 集合等价于 \(\llbracket R \rrbracket\) 的键集合）。

> 注：这里的“正确”是指键集合/键值流一致；值语义（覆盖写、版本、TTL）由第 6.3 节的 namespace 契约决定。

### 11.4 验证建议（面向项目指标）

建议把验证拆成三层，不需要引入通用 DB 基准：

- **Micro（规范化与合并收益）**：同一批请求在规范化+分组+range 合并前后，对比 RPC 数、IO ops、CPU 时间。
- **End-to-end（场景真实指标）**：
  - 推理：prefill/decode 的 P99/P999，prefetch 命中率带来的尾延迟改善
  - 训练：参数块/梯度块吞吐与 CPU overhead
  - RL：轨迹 scan 吞吐与 tail latency
- **Mixed workload（混部场景）**：混部下的扇出与尾延迟，展示“统一表示 + 确定性 lowering”降低系统抖动。
