...
export default function ChineseHome() {
  return <Layout title="Agent 时代的持久化基础设施" description="在可审查的执行边界中运行 Agent，并保存可查询历史。">
    ...
    <main>
      ...
      <section className="container padding-vert--xl quickstart-section"><div className="text--center"><h2>从安装到得到结果</h2><p className="section-lede">先完成最小闭环，保留输出，再按下一个问题选择深入指南。</p></div><div className="quickstart-commands"><article><span className="step-number">01</span><h3>安装</h3><pre><code>{"pip install 'persisting[lance]'"}</code></pre></article><article><span className="step-number">02</span><h3>运行或探索</h3><pre><code>{"pvisor run --stage ./runs/task-001 -- codex\n# 或：pchronicle onboard"}</code></pre></article><article><span className="step-number">03</span><h3>审查或查询</h3><pre><code>{"pvisor review last\n# 或：pchronicle onboard query"}</code></pre></article></div></section>
