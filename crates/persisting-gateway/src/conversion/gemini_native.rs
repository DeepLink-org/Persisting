...
/// Gemini native SSE → OpenAI Chat Completions SSE.
pub struct GeminiNativeStreamTranslator {
...
impl GeminiNativeStreamTranslator {
    ...
    pub fn push_chunk(&mut self, chunk: &[u8]) -> Result<String> {
        ...
        self.upstream_buf.push_str(&chunk);
        let mut output = String::new();
        while let Some(data) = next_sse_data(&mut self.upstream_buf) {
            if data == "[DONE]" {
                output.push_str(&self.finish_stream()?);
                continue;
            }
            let native: Value = serde_json::from_str(&data).context("parse Gemini SSE chunk")?;
            if let Some(id) = native.get("responseId").and_then(Value::as_str) {
                self.stream_id.get_or_insert_with(|| id.to_string());
            }
            if let Some(model) = native.get("modelVersion").and_then(Value::as_str) {
                self.model.get_or_insert_with(|| model.to_string());
            }
            if let Some(usage) = native.get("usageMetadata") {
                self.metrics.usage = token_usage(usage);
            }
            if let Some(event) = self.translate_event(&native) {
                output.push_str(&format!("data: {event}\n\n"));
            }
        }
        anyhow::ensure!(
            self.upstream_buf.len() <= MAX_SSE_FRAME_BYTES,
            "Gemini SSE frame exceeds {MAX_SSE_FRAME_BYTES} bytes"
        );
        Ok(output)
    }

    fn translate_event(&mut self, native: &Value) -> Option<Value> {
        ...
        if !decoded.text.is_empty() {
            ...
            self.accumulated_text.push_str(&decoded.text);
            delta.insert("content".into(), json!(decoded.text));
        }
        if !decoded.reasoning.is_empty() {
            delta.insert("reasoning_content".into(), json!(decoded.reasoning));
        }
        if !decoded.tool_calls.is_empty() {
            self.saw_tool = true;
            if self.metrics.ttft_ms.is_none() {
            ...
            let calls = decoded
                ...
                .map(|call| {
                    ...
                    self.tool_index += 1;
                    let mut call_json = tool_call_json(call, &id, index as usize);
                    call_json["index"] = json!(index);
                    call_json
                })
                .collect::<Vec<_>>();
            delta.insert("tool_calls".into(), json!(calls));
            delta.entry("content").or_insert(Value::Null);
        }
        let finish = candidate
            .and_then(|candidate| candidate.get("finishReason"))
            .and_then(Value::as_str)
            .map(|reason| finish_reason(Some(reason), self.saw_tool));
        let blocked = candidate.is_none()
            && native
                .get("promptFeedback")
                .and_then(|feedback| feedback.get("blockReason"))
                .is_some();
        let finish = finish.or(blocked.then_some("content_filter"));
        let usage = native.get("usageMetadata").map(gemini_usage);
        if delta.is_empty() && finish.is_none() && usage.is_none() {
            return None;
        }
        let id = self
            .stream_id
            .clone()
            .unwrap_or_else(|| format!("gemini-{}", self.created));
        let model = self
            .model
            .clone()
            .unwrap_or_else(|| self.fallback_model.clone());
        let choices = if delta.is_empty() && finish.is_none() {
            Vec::new()
        } else {
            vec![json!({"index":0,"delta":Value::Object(delta),"finish_reason":finish})]
        };
        let mut event = json!({
            "id":id,
            "choices":choices,
...
fn next_sse_data(buffer: &mut String) -> Option<String> {
    ...
    *buffer = buffer[position + delimiter_len..].to_string();
...
mod tests {
    ...
    #[test]
    fn stream_parser_accepts_crlf_frames() {
        let mut translator = GeminiNativeStreamTranslator::new("gemini-2.5-pro");
        let output = translator
            .push_chunk(
                b"data: {\"responseId\":\"r\",\"candidates\":[{\"content\":{\"parts\":[{\"text\":\"ok\"}]}}]}\r\n\r\n",
            )
            .unwrap();
