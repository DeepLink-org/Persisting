...
"#,
    ...
    let mut opts = common::run_opts(script);
    opts.workers = 1;
    opts.skip_task_ids = SkipSet::from_iter(["t-0".into(), "t-1".into()]);
...
"#,
    ...
    {
        let sink: Arc<dyn persisting_compute::ResultSink> =
    ...
    // Run 2: resume — nothing left to dispatch.
    {
        ...
        let mut opts = common::run_opts(script.clone());
        opts.workers = 1;
        opts.skip_task_ids = SkipSet::from_iter(ledger.skip_ids());
        opts.sink_submitter = Some(writer.submitter());
        let results = run_local_fleet(opts, |_| {}).await.unwrap();
        writer.join().await.unwrap();
        assert!(
    ...
    let mut opts = common::run_opts(script);
    opts.workers = 1;
    opts.skip_task_ids = SkipSet::from_iter(partial_ledger.skip_ids());
    opts.sink_submitter = Some(writer.submitter());
    let results = run_local_fleet(opts, |_| {}).await.unwrap();
    writer.join().await.unwrap();
