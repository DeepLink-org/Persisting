...
use persisting_compute::dist::DistEnv;
use persisting_compute::job_control::{
    broadcast_job_cancel, JobControlActor, JobControlCommand, JobControlReply,
};
use persisting_compute::pulsing_ext::{ask_timeout, resolve_actor, ASK_TIMEOUT};
use persisting_compute::{TaskExpr, WorkerActor, WorkerCommand, WorkerReply};
...
"#,
    ...
    let result: persisting_compute::TaskResult = serde_json::from_slice(&result_json).unwrap();
