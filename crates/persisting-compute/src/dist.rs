...
//!
//! ```bash
//! torchrun --nproc_per_node=4 -- persisting compute plan.py
//! ```
...
impl DistEnv {
    ...
    pub fn slot_name(worker: usize, slot: usize, per_worker: usize) -> String {
        if per_worker <= 1 {
            ...
            format!("compute/worker/{worker}/slot/{slot}")
        }
    }
    ...
    pub fn job_control_name(rank: usize) -> String {
        format!("compute/job_control/{rank}")
    }
}
...
mod tests {
    ...
    fn slot_names_slot_major() {
        assert_eq!(
            ...
            vec![
                "compute/worker/0/slot/0",
                "compute/worker/1/slot/0",
                "compute/worker/0/slot/1",
                "compute/worker/1/slot/1",
            ]
        );
        assert_eq!(
            DistEnv::slot_names(2, 1),
            vec!["compute/worker/0", "compute/worker/1"]
        );
    }
