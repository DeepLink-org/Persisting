//! Typed pChronicle search and trajectory request/response values.
...
pub struct SearchImportLanceRequest {
    ...
    /// Path or URI accepted by pChronicle's Lance dataset loader.
...
pub struct JudgeScoreInput {
    /// Turn scope: dialogue `call_id`. Story scope: omit (pChronicle uses `__story__`).
    #[serde(default)]
