...
impl AttemptRegistry {
    pub async fn open(root: impl AsRef<str>) -> anyhow::Result<Self> {
        Ok(Self {
            store: CasStore::open(root, ATTEMPT_DIR, "Attempt registry").await?,
        })
    }
    ...
    where
        T: Send + 'static,
        F: Fn(Option<&AttemptRecord>) -> anyhow::Result<Mutation<AttemptRecord, T>>
            + Send
            + Sync
            + 'static,
    {
        self.store.mutate(run_id, mutate).await
    }
}
