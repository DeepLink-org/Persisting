//! Search-specific Lance storage: append rows, IVF-PQ index, IVF physical reorder,
...
pub async fn reorder_ivf(request: SearchIndexReorderRequest) -> Result<SearchIndexReorderResponse> {
    ...
    Ok(SearchIndexReorderResponse {
        ...
        in_place: request.in_place,
        status: "ok".to_string(),
        note: "IVF physical reorder (pChronicle remap path; indices remapped, PQ not retrained)."
            .to_string(),
        log,
    })
}
