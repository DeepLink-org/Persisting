...
mod tests {
    ...
    mod proptests {
        ...
        proptest! {
            ...
            #[test]
            fn parse_json_value_roundtrips_json_and_falls_back_for_raw_text(value in object_strategy(), raw in "[a-zA-Z0-9 _-]{1,32}") {
                prop_assert_eq!(parse_json_value(&value.to_string()), value);
                let parsed = parse_json_value(&raw);
                if serde_json::from_str::<Value>(&raw).is_err() { prop_assert_eq!(parsed, Value::String(raw)); }
            }

            #[test]
            fn text_helpers_concatenate_only_text_bearing_parts(parts in proptest::collection::vec(text_part_strategy(), 0..24)) {
                let expected = parts.iter().filter_map(part_text).filter(|text| !text.is_empty()).collect::<Vec<_>>().join("");
                ...
                let expected_leftovers = parts.iter().enumerate().filter(|(_, part)| part_text(part).is_none()).map(|(index, part)| (index, part.clone())).collect::<Vec<_>>();
                prop_assert_eq!(leftovers, expected_leftovers);
            }

            #[test]
            fn empty_or_whitespace_only_lines_are_ignored(blank_lines in proptest::collection::vec(any::<bool>(), 0..16)) {
                let input = blank_lines.iter().map(|blank| if *blank { " \t" } else { "" }).collect::<Vec<_>>().join("\n");
                let count = for_each_jsonl_object(Cursor::new(input), |_, _| Ok(())).unwrap();
