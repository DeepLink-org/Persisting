...
mod tests {
    ...
    mod proptests {
        ...
        proptest! {
            ...
            #[test]
            fn generated_event_logs_keep_all_input_events(timestamps in proptest::collection::vec(any::<String>(), 0..12)) {
                let events = timestamps.into_iter().map(|timestamp| json!({"timestamp": timestamp})).collect::<Vec<_>>();
                let trajectory = ActfTrajectory::from_event_log(events.clone());
                prop_assert_eq!(trajectory.events, events);
            }

            #[test]
            fn actf_path_hints_are_case_insensitive(
                suffix in proptest::string::string_regex("[A-Za-z0-9_-]{1,24}").unwrap(),
            ) {
                let path = format!("task-{suffix}.AcTf.JsOn");
                prop_assert_eq!(
                    ActfFormat.probe(Some(Path::new(&path)), b"\n").unwrap(),
