...
fn search_end_to_end_write_index_query_vector_and_fts() {
    ...
    assert!(
        names.contains(&persisting_pchronicle::PERSISTING_VECTOR_INDEX_NAME),
        "expected IVF-PQ index name in {:?}",
        names
    );
    assert!(
        names.contains(&persisting_pchronicle::PERSISTING_FTS_INDEX_NAME),
...
fn search_index_delete_removes_named_index() {
    ...
    let del = search_index_delete(SearchIndexDeleteRequest {
        dataset: dataset.clone(),
        index_name: persisting_pchronicle::PERSISTING_FTS_INDEX_NAME.into(),
    })
    .unwrap();
