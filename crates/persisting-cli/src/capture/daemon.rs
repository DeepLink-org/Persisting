...
use persisting_gateway::session::index::{discover_sessions, SessionSummary};
...
pub fn cmd_start(opts: StartOptions) -> Result<()> {
    ...
    cmd.args(args);
    if opts.debug {
        cmd.env(persisting_gateway::runtime::debug::ENV_CAPTURE_DEBUG, "1");
    }
    ...
    if opts.debug {
        eprintln!(
            ...
            persisting_gateway::runtime::debug::ENV_CAPTURE_DEBUG
        );
    }
