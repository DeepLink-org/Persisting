...
use crate::markdown_trajectory::format_duration_human;
use crate::session_index::{SessionIndexStore, SessionSummary};
use crate::session_storage::{trajectory_run_dir, CaptureRoute};
use persisting_pchronicle::{
    encode_agenticmd_preamble, is_subagent_session_storage_key,
    read_agenticmd_blocks_from_file as read_blocks_from_file, AGENTICMD_BLOCK_LAYOUT,
...
pub fn refresh_document_frontmatter(
    ...
    let body_start = persisting_pchronicle::agenticmd_body_byte_offset(&content)
        .map_err(|e| anyhow::anyhow!("pchronicle agenticmd body offset: {e}"))?;
