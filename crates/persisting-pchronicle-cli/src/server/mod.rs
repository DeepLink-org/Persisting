...
impl PreparedWarehouse {
    ...
    async fn install_catalog_runtime(&self, runtime: Arc<CatalogRuntime>) -> String {
        ...
        *self.state.trajectory_cache.write().await = None;
        self.state.scoped_queries.invalidate();
        snapshot_id
    }
...
async fn current_catalog_for_runs(
    ...
    if let Ok(mut refresh) = state.catalog_refresh.clone().try_lock_owned()
        && *refresh <= Instant::now()
        && let Ok(permit) = query_admission::REFRESH_SLOT.try_acquire()
    {
        let state = state.clone();
        let current = runtime.clone();
        tokio::spawn(async move {
            let _permit = permit;
            rebuild_catalog_for_runs(&state, current).await;
            *refresh = Instant::now() + state.catalog_refresh_interval;
        });
    }
    Ok(runtime)
}

async fn rebuild_catalog_for_runs(
    state: &AppState,
    current: Arc<CatalogRuntime>,
) -> Arc<CatalogRuntime> {
    let current = state.catalog.read().await.clone().unwrap_or(current);
    if current.built_at.elapsed() < state.catalog_refresh_interval {
        return current;
    }
    // Publish only after summaries are readable too: discovery alone can succeed
    // while a lazily opened source is broken.
    let refreshed = async {
        let runtime = build_catalog_runtime(&state.config).await?;
        runtime
            .acceleration
...
async fn catalog(
    ...
    let runtime = current_catalog(&state, &request_id).await?;
    metrics.record("catalog", started);
    ...
    if let Some(dataset) = dataset {
        ...
        let result = result
            .map(|(summaries, cache_status)| {
                if let Some(metrics) = metrics {
                    metrics.record(cache_status, started);
                }
                summaries.as_ref().clone()
            })
            .map_err(|error| fail(request_id, "load_run_summaries", error));
        if let Some(metrics) = metrics {
            metrics.record("summary_admission", started);
        }
        return result;
    }
    let phase = Instant::now();
...
/// Storyline store under the mount, list document IDs directly from CURRENT.
async fn try_on_demand_storyline_runs_page(
    state: &AppState,
    query: &explorer::ExplorerRunsQuery,
    request_id: &RequestId,
) -> Result<Option<explorer::RunExplorerPage>, ApiError> {
    ...
    let Some((_generation, ids)) = store
        ...
        let limit = query.limit.unwrap_or(50).clamp(1, 200);
        return Ok(Some(explorer::RunExplorerPage {
            snapshot: explorer::PageSnapshot {
                offset: 0,
                next_offset: 0,
                total: 0,
    ...
    let limit = query.limit.unwrap_or(50).clamp(1, 200);
    let total = ids.len();
    let page_ids = ids.into_iter().skip(offset).take(limit).collect::<Vec<_>>();
    let page_records = page_ids
        .into_iter()
        .map(|document_id| {
            let path = explorer::explorer_run_path(
                dataset_name,
    ...
    Ok(Some(explorer::RunExplorerPage {
        ...
        records: page_records,
        path_index,
        search: explorer::RunSearchStatus {
            fts_available: false,
            mode: "none",
            tokenizer: None,
        },
...
async fn explorer_runs(
    ...
    let dataset_filter = query
        ...
        .filter(|value| !value.is_empty() && *value != "all");
    let started = Instant::now();
    let summaries = load_run_summaries(
        &state,
        dataset_filter,
        query.file.as_deref(),
        &request_id,
...
async fn resolve_run_summary(
    ...
    // Exact Storyline leaves can identify a run with one document lookup;
    // avoid scanning the whole source just to build a summary row.
    if query
        .dataset
        .as_deref()
        ...
        && query
            ...
            .as_deref()
            .is_some_and(|value| !value.trim().is_empty())
        && !query.session_id.trim().is_empty()
        && let Some(run) = try_resolve_on_demand_storyline_run(state, query, request_id).await?
    {
        return Ok(run);
    }
    let dataset_filter = query
        ...
        .map(str::trim)
        .filter(|value| !value.is_empty() && *value != "all");
    let mut matches = load_run_summaries(
        state,
        dataset_filter,
        query.file.as_deref(),
        request_id,
    ...
    if let Some(metrics) = metrics {
        metrics.record("resolve_filter", filter_started);
    }
    if matches.is_empty() {
...
/// Synthesize a RunSummary for a nested Storyline leaf that is reachable under
/// a Directory mount but absent from the catalog snapshot.
async fn try_resolve_on_demand_storyline_run(
    state: &AppState,
    query: &SessionQuery,
    request_id: &RequestId,
    ...
    Ok(Some(RunSummary {
        ...
        run_id: query.run_id.clone().filter(|value| !value.is_empty()),
        agent_id: if query.agent_id.trim().is_empty() {
            "storyline".into()
        } else {
...
async fn load_on_demand_storyline_bundle(
    ...
    // Never shadow a catalog-registered leaf source with a direct open.
    if dataset.sources.iter().any(|source| {
        source.kind != persisting_pchronicle::storage::CatalogSourceKind::Directory
    ...
    let under_directory = dataset.sources.iter().any(|source| {
        source.kind == persisting_pchronicle::storage::CatalogSourceKind::Directory
            && (run.file == source.file || run.file.starts_with(&format!("{}/", source.file)))
    });
    if !under_directory {
        return Ok(None);
    }
    let uri = format!(
        "{}/{}",
        dataset.mount.uri.trim_end_matches('/'),
        run.file.trim_matches('/')
    );
    let store = persisting_pchronicle::storage::StorylineLanceStore::open_uri(&uri)
        .await
        .map_err(|error| fail(request_id, op, error))?;
    let document_id = if run.document_id.is_empty() {
...
async fn explorer_turns(
    ...
    metrics.record("turn_fts_probe", phase);
    ...
    {
        if paths.is_none() {
            // On-demand nested Storylines are not registered in DuckDB; filter
            // the already-loaded turns in memory instead of SQL FTS.
            search_mode = "memory";
            ...
            fts.extend(fts_errors);
            metrics.record("turn_search", phase);
...
fn query_table_summaries() -> Vec<QueryTableSummary> {
    vec![
        QueryTableSummary {
            name: "sources",
...
struct QueryTablesQuery {
    /// Accept both `ui=true` (new clients) and `ui=1` (cached clients).
    #[serde(default)]
    ui: Option<String>,
}
...
async fn ui_query_catalog(state: &AppState) -> Result<Json<QueryCatalog>, ApiError> {
    ...
    let storage_path = state
        ...
        .datasets
        .iter()
        .find(|mount| mount.name == default_name)
        ...
        let (ready_sources, error_sources) = tree
            .map(|tree| {
                tree.children.iter().fold((0, 0), |(ready, failed), child| {
                    (
                        ready + usize::from(child.kind == "file"),
                        failed + usize::from(child.failed_count > 0),
                    )
                })
            })
            .unwrap_or_default();
...
async fn query_tables(
    State(state): State<AppState>,
    request_id: RequestId,
    Query(query): Query<QueryTablesQuery>,
    ...
    {
        return ui_query_catalog(&state).await;
    }
    let runtime = current_catalog(&state, &request_id).await?;
...
async fn compile_analysis(
    ...
    if request.snapshot_id != "ui-cache" && request.snapshot_id != runtime.snapshot.snapshot_id() {
