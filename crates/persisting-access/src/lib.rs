//! Shared network/model access policy decisions.
//!
//! Used by pVisor (control/runtime) and Capture (HTTP proxy adaptation).
//! Kept in a separate crate so Capture and pVisor can both depend on it without
//! a dependency cycle when pVisor owns Capture session lifecycle.
