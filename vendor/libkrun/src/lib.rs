...
/// Add a copy-on-write virtio-fs device without creating a host FUSE mount.
///
/// `lower_paths` are ordered from highest to lowest precedence. Guest writes
/// are stored in `upper_path`; whiteouts use the portable `.wh.*` format.
#[allow(clippy::missing_safety_doc, clippy::too_many_arguments)]
#[unsafe(no_mangle)]
#[cfg(not(any(feature = "tee", feature = "aws-nitro")))]
pub unsafe extern "C" fn krun_add_virtiofs_overlay(
    ctx_id: u32,
    c_tag: *const c_char,
    ...
    let parse = |pointer: *const c_char| {
        if pointer.is_null() {
            return Err(());
        }
    ...
    let tag = match parse(c_tag) {
        Ok(value) => value,
        Err(()) => return -libc::EINVAL,
    };
    let upper_dir = match parse(c_upper_path) {
        Ok(value) => value,
        Err(()) => return -libc::EINVAL,
    };
    let work_dir = if c_work_path.is_null() {
        None
    } else {
    ...
    let lowers = slice::from_raw_parts(lower_paths, lower_count)
        .iter()
        .map(|pointer| parse(*pointer))
        .collect::<Result<Vec<_>, _>>();
    let lower_dirs = match lowers {
        Ok(value) => value,
        Err(()) => return -libc::EINVAL,
    };
    let excluded = if excluded_count == 0 {
        ...
        match slice::from_raw_parts(excluded_paths, excluded_count)
            .iter()
            .map(|pointer| parse(*pointer))
            .collect::<Result<Vec<_>, _>>()
        {
...
mod test_disable_implicit_init {
    ...
    #[test]
    fn test_add_virtiofs_overlay_validates_and_records_config() {
        let ctx = krun_create_ctx() as u32;
